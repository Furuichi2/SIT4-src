...
//2018-10-29KOMA 高速描画復活[2/2]
#if(1)
// H2W 2017/6/20版を移植
//	高速描画（塗りつぶし）判定 ///////////////////////////////////////////////		2017/6/20 有効にする
	if ((SEQ_PAR_101 & BIT_06) != 0){				/* ﾙｰﾚｯﾄ高速表示許可？	*/
		if (PVP_CYCLE_SPM1 > SYS_HIGHSPM_B1) {		/* SPM HIGHｼｷｲ値以上？	*/
			RULET_HIGH_FLG = ALL_DISP;				/* 高速ﾙｰﾚｯﾄ描画ON		*/
		}else {
			if (RULET_HIGH_FLG == ALL_DISP) {
				if (PVP_CYCLE_SPM1 <= SYS_LOWSPM_B1) {	/* SPM LOWｼｷｲ値以下？	*/
