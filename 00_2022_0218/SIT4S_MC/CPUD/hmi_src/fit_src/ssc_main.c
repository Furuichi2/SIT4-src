...
extern	UNs		GMN_PAR105;				//パスワード		2017/11/06
...
void	FRAM_copy(void);						//FRAMのｻﾑ対象ｴﾘｱをｺﾋﾟ-		2016/06/16
...
void	dsp_init1(void)
{
	...
	if (SV_SUM_TEST != 0x12345678) {		//ｻﾑ異常網掛け用			2016/06/16
		FRAM_copy();
		SV_SUM_TEST = 0x12345678;

		poi = (UNs *)&SV_SUM_TEST_DAT;
		for(chk=0; chk<(128/2); chk++) {
			poi[chk] = 0;
		}
	}
...
//V06n
	...
	SYS_CBdata_read();								//シスパラよりクラッチ設定データ読み込み	2016/02/29
...
void	Any_mng2(void)
{
	...
	FRAM_copy();										//サム異常網掛け用					2016/06/16

	if (GMN_PAR105 == 0) {								//パスワード確認					2017/11/06
		SEQ_PAR_058 |= BIT_5;							//パスワード機能無効＝パスワード一致扱いとする。
	}
	else {
		if (GMN_PAR105 == SV_PASSWORD) {
			SEQ_PAR_058 |= BIT_5;						//パスワード一致
		}
		else {
			SEQ_PAR_058 &= ~BIT_5;						//パスワード不一致
		}
	}
}
...
void	FRAM_csum_set(void)
{
	...
	SV_BACK_SUM = scode;						//2016/06/16

	SEQ_PAR_099++;								//2015/09/20
}
...
void	FRAM_csum_chk(void)
{
	...
	UNi		cnt;
	UNs		*src1_w, *src2_w;
	UNs		err_adr[8], src1_dat[8], src2_dat[8], i, j;
	...
	if(SUM_CHK != scode ){						//設定値サムチェックコード チェック
		...
		src1_w = (UNs *)&SEQ_016_SV;
		src2_w = (UNs *)&SV_BACK_AREA;
		for (i=0, j=0; i<(0x1400/2); i++, src1_w++, src2_w++) {
			if (*src1_w != *src2_w) {
				err_adr[j] = (UNs)((UNl)src1_w & 0x0ffff);
				src1_dat[j] = *src1_w;
				src2_dat[j] = *src2_w;
				j++;
				if (j>=8) {
		...
		src1_w = (UNs *)&SV_SUM_TEST_DAT;
		for(j=0; j<8; j++) {
			src1_w[j*3+0] = err_adr[j];
			src1_w[j*3+1] = src1_dat[j];
			src1_w[j*3+2] = src2_dat[j];
		}
	}
}
...
//2020-09-18	FRAM_csum_set();				//ﾌﾗｼｭRAM領域の設定値のﾁｪｯｸｻﾑ･ｺｰﾄﾞを作成
//2020-09-18	FRAM_KAT_csum_set();			//ﾌﾗｼｭRAM領域の型ﾃﾞｰﾀ 名称＆メモのﾁｪｯｸｻﾑ･ｺｰﾄﾞを作成
...
void	FRAM_copy(void)
{
	...
	src_p = (UNl *)&SEQ_016_SV;					//ﾌﾗｼｭRAM領域の設定値 ﾄｯﾌﾟｱﾄﾞﾚｽ
	dst_p = (UNl *)&SV_BACK_AREA;

	for(cnt=0x1400/4; cnt!=0; cnt--,src_p++,dst_p++){	//fram_top+0x4800～0x5C00にｺﾋﾟｰ(0x1400ﾊﾞｲﾄ)
		*dst_p = *src_p;
	}
	SV_BACK_SUM = SUM_CHK;							//設定値サムチェックコード
}
