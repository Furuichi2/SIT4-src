...
extern	UNs		GMN_PAR111;						//外部タイマ上限２	2020/01/30 V01l


//2020/05/07(V01m)(2019-04-20) 荷重オーバロードの設定上限追加 設定上下PARAM<最大荷重
extern	UNs		GMN_PAR106;						//2020/05/07(2019-04-20) ｵｰﾊﾞﾛｰﾄﾞ左右,ﾛｱﾘﾐｯﾄ左右,振幅の設定上限
extern	UNs		GMN_PAR107;						//2020/05/07(2019-04-20) ｵｰﾊﾞﾛｰﾄﾞ合計,ﾛｱﾘﾐｯﾄ合計,振幅の設定上限
...
UNs		Key_sv_limt_chk(UNs t_no, UNl *wk_p)
{
	...
	UNs		i , j, ww0, ww1;
//2016/02/25	UNl		*Dst_p_w, long_bf, *posn, *posb;
	UNl		*Dst_lp;
	UNs		*Dst_wp;
...
//2014/12/22					*Dst_p = (SV0_DAIHAI_LNG1O*100) / wl0;
		...
		case 21:				//オーバーロード（左）　ＭＡＸチェック
			if (SV_OVLD_SEL != 1) {										//2014/06/03
				/*「最大値」または「振幅」*/
				if( (bf & 0xffff0000) == 00 ) {
					//2020/05/07(V01m)2019-04-20 koma ｵｰﾊﾞﾛｰﾄﾞ左右,ﾛｱﾘﾐｯﾄ左右,振幅の設定上限
					if(GMN_PAR106){
						if(bf > GMN_PAR106){
							bf=GMN_PAR106;
							*Dst_p=bf;
		...
		case 22:				//オーバーロード（合計）　ＭＡＸチェック
			if (SV_OVLD_SEL != 1) {										//2014/06/03
				/*「最大値」または「振幅」*/
				if( (bf & 0xffff0000) == 00 ) {
					//2020/05/07(V01m)2019-04-20 koma ｵｰﾊﾞﾛｰﾄﾞ合計,ﾛｱﾘﾐｯﾄ合計,振幅の設定上限
					if(GMN_PAR107){
						if(bf > GMN_PAR107){
							bf=GMN_PAR107;
							*Dst_p=bf;
...
////			}
		...
		case 109:				//外部タイマ	V05r
			//2020-01-30 V01lｼｰｹﾝｽでの外部ﾀｲﾏ上限切替 GMN_PAR111追加 <=>GMN_PAR041
...
//2014/12/22			bf = RnaDigChange1(SV0_FIXARA_AGLE, bf);		//mm変換→角度
		...
		case 155:				//校正荷重  最大荷重（左）以下　ＭＡＸチェック		2016/02/25
			if (SV0_KOUKAJ_ENB != 1) {
				Dst_wp = SV_buff[t_no].DATA_ADR;				//ﾃﾞｰﾀ格納ｱﾄﾞﾚｽ
				*Dst_p = *Dst_wp;
			}
		...
		case 156:				//校正荷重  最大荷重（合計）以下　ＭＡＸチェック	2016/02/25
			if (SV0_KOUKAJ_ENB != 1) {
				Dst_wp = SV_buff[t_no].DATA_ADR;				//ﾃﾞｰﾀ格納ｱﾄﾞﾚｽ
				*Dst_p = *Dst_wp;
			}
		...
		case 157:				//校正荷重  最大荷重（右）以下　ＭＡＸチェック		2016/02/25
			if (SV0_KOUKAJ_ENB != 1) {
				Dst_wp = SV_buff[t_no].DATA_ADR;				//ﾃﾞｰﾀ格納ｱﾄﾞﾚｽ
				*Dst_p = *Dst_wp;
			}
