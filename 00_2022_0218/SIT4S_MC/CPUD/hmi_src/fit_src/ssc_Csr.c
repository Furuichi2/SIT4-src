...
//V02a				if( (SEQ_020_SV & BIT_8) && (SEQ_PAR_385 & BIT_7) )		//荷重計有＆荷重計表示指令有	/* V01m */
		...
		case 108:	//下限位置教示							//2017/11/08
			if ( (SEQ_050_SV & BIT_9) == 0 ) {				//荷重補正無し？
				if (CMD_DAIPOS_DSP1) {						// 0=非表示 1=表示  表示しているときだけ教示有効  /* V01c */
					i = SV_TBL_POS_NOW;
					DYHITO_POS_Teaching();					//
					mmon_trans_put();						//回転方式によるﾃﾞｰﾀの格納
...
//2017/11/09	SVPV_data_disp(cnv_bf, &SV_buff[t_no], z_f, flag);	//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽ/正常表示)
	SVPV_data_disp(*Dst_lp, &SV_buff[t_no], z_f, flag);	//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽ/正常表示)
