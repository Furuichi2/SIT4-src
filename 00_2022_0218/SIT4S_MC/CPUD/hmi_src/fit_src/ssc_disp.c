...
//V06n				SEQ_PAR_051 = BIT_2;			//ﾃﾞｰﾀ設定画面表示(1:表示中)
				...
				if ( (SEQ_PAR_058 & BIT_5) == 0 ) {		//パスワード不一致		2017/11/06
					SV_CNT = 00;
					CSR_CNT = 02;
				}
			}
...
U2b		MOT_INF_GET(void)
{
	...
	U2b	stopcnt=0;
//V01m 2020/05/07	U2b	stopcntmax;
	U2b	stopcntmax=0;//V01m 2020/05/07(2020/02/24) koma 待機点切替-進角設定できない不具合
	U2b	ans=0;
	int	i;
	...
	if((1<stepmax)&&(stepmax<=11)){//1~10→2~11
		...
		for(i=0;i<stepmax;i++){
			...
			//V01m  : 2020/05/07(2020/02/24) i==0追加
			if(i==0){
				//回転の待機点0以外対策
				dir=0;//1段目は必ず正転
			}else{
