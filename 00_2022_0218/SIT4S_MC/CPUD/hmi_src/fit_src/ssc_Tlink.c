...
***********************************************************************
*
*  Module Name:		TLINK_NEXT_READ
*  Function Name:	Ｔ－ＬＩＮＫ外部型番号読込処理
*  Input	:		なし
*  Return	: 		なし
*  Note		:
*
...
***********************************************************************
*/
void	TLINK_NEXTREAD_sub(void)
{
	UNs	next_no, *adr;

	if (KAT_DATCHG_FLG != 0)					//型ﾃﾞｰﾀ変更有り？
	{											//はい
	}

	next_no = TLINK_NEXT_NO;
	if ( NEXT_MLDSEQ_FLG == 0xff )
	{
		next_no = (SEQ_PAR_112 & 0x00ff);
	}

	if ( (next_no != 0) && (next_no <= 200) )
	{
		NEXT_DATA_READ(next_no);

		if(SEQ_PAR_078 & BIT_0)
		{
			SEQ_PAR_047 |= BIT_13;						// 異常終了
		}
		else
		{
			NEXT_READ_AFTER();							/* 次期読み込み後の再表示 		*/
			SEQ_PAR_047 |= BIT_14;						// 正常終了
		}
	}
	else
	{
...
void	TLINK_NEXT_READ(void)
{
	UNs	dat, chg, one, ofe;

	NEXT_MLDSEQ_FLG = 0xff;						//シーケンス用次型データ読み込みフラグＯＮ

	dat = SEQ_PAR_103;
	chg = TLINK_NEXT_LAST1 ^ dat;			// 変化
	one = chg & dat;						// ONｴｯｼﾞ
	ofe = chg & TLINK_NEXT_LAST1;			// OFFｴｯｼﾞ
	TLINK_NEXT_LAST1 = dat;

	if (one & BIT_6)							// シーケンスからの型番号切替指令
	{
		TLINK_NEXTREAD_sub();
	}
	else
	{
		NEXT_MLDSEQ_FLG = 0;					//シーケンス用次型データ読み込みフラグクリア

		if ( (SEQ_020_SV & BIT_12) == 1)	// ＴＬＩＮＫ仕様あり？ 2009/03/27(V00n_e)
		{
			dat = SEQ_PAR_103;
			chg = TLINK_NEXT_LAST ^ dat;			// 変化
			one = chg & dat;						// ONｴｯｼﾞ
			ofe = chg & TLINK_NEXT_LAST;			// OFFｴｯｼﾞ
			TLINK_NEXT_LAST = dat;
			if ( (SEQ_020_SV & BIT_12) && (one & BIT_3) )		// Ｔリンクからの型番号切替指令
			{
				TLINK_NEXTREAD_sub();
			}
		}
...
TLINK_NEXTREAD_900:
	if ( (SEQ_PAR_103 & (BIT_3 | BIT_6)) == 0 )
	{
		SEQ_PAR_047 &= ~(BIT_13 | BIT_14);
	}
}
