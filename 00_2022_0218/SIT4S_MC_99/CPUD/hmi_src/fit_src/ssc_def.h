...
#define		E_CPUC_VER1	'A'							//CPU-C VER. No.
#define		E_CPUC_VER2	'0'							//CPU-C VER. No.
#define		E_CPUC_VER3	'1'							//CPU-C VER. No.
#define		E_CPUC_VER4	'v'							//CPU-C VER. No.
...
//V01e	: 2015/09/29	モーション100段対応、SUM異常対策(V01d)、inch対策(V01c)
//		: 2015/10/21	モーション100段対応
//		: 2015/11/02	SUM異常対策(ﾓｰﾄﾞ切替時のｻﾑ作成削除)
//		: 2015/11/26	SUM異常対策(型ﾃﾞｰﾀのﾒﾓ登録時)
//		: 2016/01/12	ｽﾗｲﾄﾞ自動調節の登録を有効にする。
//		: 2016/01/14	DPRAMのｼｰｹﾝｽ領域ﾏｯﾌﾟ変更によりｼﾘｱﾙ内部読出しﾌﾟﾛｸﾞﾗﾑを変更する。
//V01f	: 2016/02/24	SIT4画面修正仕様対応
//V01g	: 2016/03/24	レベラフィーダ対応、99段前で修正した内容をフォロー
//A01h	: 2016/04/15	モーション変速設定追加
//A01i	: 2016/04/22	周辺機連動機能追加仕様
//A01j	: 2016/09/17	多段モーション設定追加
//A01k	: 2016/11/01	2016/05/**, 2016/07/09	補正モニタ表示中の出力ビット不具合修正（V01f：周辺機連動機能追加から発生！）
//A01l	: 2016/11/14	ロータリカムタイマ仕様追加
//A01m	: 2017/05/10	現在角度、ストッピングタイム、ブレーキ角度、ブレーキタイムをFERAMに割付（上位通信読出し用）
//A01n	: 2017/08/10	反転モード設定が出来ない不具合修正
//		: 2017/08/21	99段の単位切替対応
//A01o  : 2018/09/29    高速ルーレット復活
//A01p  : 2018/11/18    test
//A01q  : 2019/04/20    荷重リミット
//A01r  : 2019/06/07    待機点切替の振子,反転への影響なくす
//A01s  : 2020/01/30    koma 外部タイマの上限切替
//A01t  : 2020/02/24    koma 進角設定不可機能 待機点0度以外でも可能にする ssc_disp.c
//A01u  : 2020/09/01    koma サム演算修正
//A01v  : 2021/06/24    koma ADC
...
#define		E_sstg_lfeed	0x1b		//生産[フィーダ設定]へ				//2016/03/18
//段取の管理ｽﾃｰｼﾞ数
...
#define		E_dstg_msset	0x23		//データ設定[多段モーション設定]へ		/* SIT4-MC */		2016/09/17
//ｼｽﾃﾑ設定の管理ｽﾃｰｼﾞ数
...
#define		mot100_test		1


//	2015-11-20 ｼｽﾃﾑの設定段数最大(KOMA)
#define		SYSPOS_STEP_MAX	98		//2015-11-20 追加 98段+1段(戻)=99段MAX


//;	******** フィーダROMパラ相対アドレス ***********		2016/03/18
#define		def_FORM_FED_NAME		0		//型式名称16バイト
#define		def_FORM_FED_SPDMX		8		//1000mm/sec 最高速度
#define		def_FORM_FED_ACCLT		9		//500msec 加速時間
#define		def_FORM_FED_LDYTM		10		//30msec  リリース(L)反応遅延(DLY)
#define		def_FORM_FED_HOSSL		11		//矯正量自動調整有(1)無(0)
#define		def_FORM_FED_DIRCT		12		//ﾓｰﾀ軸PG極性
#define		def_FORM_FED_PLS_N		13		//換算比率 "N"100 PLS
#define		def_FORM_FED_LNG_M		14		//換算比率 "M"12.3mm
#define		def_FORM_FED_INPPS		15		//ｲﾝﾎﾟｼﾞｼｮﾝ 500pls
#define		def_FORM_FED_SYLNG		16		//遮光時材料移動量 10.2mm [0.1mm]
#define		def_FORM_FED_HSIUP		17		//補正(HOS)入口(In)上限(UP)   9.99[0.01mm]
#define		def_FORM_FED_HSIDN		18		//補正(HOS)入口(In)下限(DN)  -9.99
#define		def_FORM_FED_HSOUP		19		//補正(HOS)出口(Out)上限(UP)  9.99
#define		def_FORM_FED_HSODN		20		//補正(HOS)出口(Out)下限(DN) -9.87
#define		def_FORM_FED_INPTM		21		//ﾌｨｰﾀﾞ余裕時間[1ms]
#define		def_FORM_FED_HENSA		22		//出力パルス偏差異常 0,1~30000PLS[1PLS]
#define		def_FORM_FED_STPE1		23		//停止時ﾓｰﾀ軸移動異常   0,1~300.00mm[0.01mm]
#define		def_FORM_FED_STPE2		24		//停止時別置ENC移動異常 0,1~300.00mm[0.01mm]
#define		def_FORM_FED_CMPE1		25		//停止中ﾓｰﾀ軸と別設置一致許容量0,1~300.00mm[0.01mm]
#define		def_FORM_FED_CMPE2		26		//動作中ﾓｰﾀ軸と別設置一致許容量0,1~300.00mm[0.01mm]
#define		def_FORM_FED_YB1		27		//
#define		def_FORM_FED_YB2		28		//
#define		def_FORM_FED_YB3		29		//
#define		def_FORM_FED_YB4		30		//
#define		def_FORM_FED_YB5		31		//
...
//#define		SUMCHK_PCAL_OFF		DEBUG_YES	//1:電源off時にｻﾑﾁｪｯｸ演算しない
//出荷用は以下
#define		SUMCHK_PCAL_OFF		DEBUG_NO	//1:電源off時にｻﾑﾁｪｯｸ演算しない
