...
extern	UNs		GMN_PAR095;						//多段モーション設定有効				2016/09/17
...
#if (mot100_test == 1)
void	MOT_NOSTEP_DISP_DI(void);				//不要行程の非表示処理
void	Make_RepStep_Bit(void);					//リピート行程をBIT情報へ
void	mmon_page_char_clr(void);				//モーション画面  SW5：前頁、SW6：次頁　文字消去
void	mmon_ud_char_clr(void);					//変速前／後消去			2016/04/19
#endif
...
#if (mot100_test == 1)
	DISP_step_csr_name();					//モーション行程No.カーソル表示
#endif
...
//2016/01/25			if(SW14_ON6_FLG < 11){
			...
			else if (SW14_ON6_FLG == 16) {			//2016/01/25
				CSR_FLG = 5;						//カーソル表示位置指定
				CSR_FLG_CNT = 5;					//カーソル番号セット
			}
			else{
				CSR_FLG = 5;						//カーソル表示位置指定
//2016/01/25				CSR_FLG_CNT = CSR_FLG_CNT_LST;		//カーソル番号セット
				CSR_FLG_CNT = 1;					//カーソル番号セット
			}

//2016/01/25			SV_FLG_CNT = 0;							//設定値番号セット
			SV_FLG_CNT = 1;							//設定値番号セット
...
//2016/09/17				if ( (GMN_PAR077 == 0) && (GMN_PAR079 != 1) ) {			//繰り返し or 一定領域				2014/09/10
				if ( (GMN_PAR077 == 0) && (GMN_PAR079 != 1)  && (GMN_PAR095 != 1) ) {			//繰り返し or 一定領域 or 多段ﾓｰｼｮﾝ
													//ＳＷ７上
...
//V06			PV0_INTER_VER2 = SV_INTER_VER2 << 8;
		...
		case 38:		//ｼｽﾃﾑ設定　C/Bｼｽﾃﾑ定数			2016/02/24
		case 51:		//ｼｽﾃﾑ設定　C/Bｼｽﾃﾑ定数2		2016/02/24
			CSR_CNT = 00;
			SV_CNT = 00;
			SW_CNT = 00;
			break;
...
//2016/02/24		case 38:		//ｼｽﾃﾑ設定　C/Bｼｽﾃﾑ定数
...
//2016/02/24		case 51:		//ｼｽﾃﾑ設定　C/Bｼｽﾃﾑ定数2
...
//V06n			SEQ_PAR_051 = BIT_2;			//ﾃﾞｰﾀ設定画面表示(1:表示中)
		...
		case 110:		//多段モーション設定画面				2016/09/17
			SW16data_onset(0xff, 9);		//ＳＷ１～６の反転指定を行う(ﾊﾟﾀｰﾝﾃﾞｰﾀ,SW位置)
			
			PV0_MSSET_STG1 = 0;				//設定ステージ初期化
			PV0_MSSET_MES1 = 0;				//メッセージ初期化
			SW56_dncara_disp(44, 8);		//ＳＷ６「設定」
			break;
...
//2016/04/22		case 18:		//段取画面　MB選択操作				//V01r
		...
		case 12:	/*V01j*/	//段取画面　ｽﾃｯﾌﾟﾓﾆﾀ
			...
			ADC_DISP_FLG = 0x0FF;				/* ＡＤＣウインドウ表示中 */		//2016/05/21
			ADC_CHG_LST1 = 0;														//
			ADC_CHG_LST2 = 0;														//
			ADC_CHG_LST2_2 = 0;														//
...
//2016/02/24			if( 00 == (SEQ_PAR_107 & BIT_2) ) {	//機能選択3(生産数値設定　1:有)		//V05g
		...
		case 25:		//生産画面　フィーダ設定	2016/03/18
			if( 00 == (SEQ_PAR_107 & BIT_2) ) {		//機能選択3(生産数値設定　1:有)
				CSR_CNT = 00;
				SV_CNT = 00;
				SW_CNT = 00;
			}
			SW16data_onset(0xff, 5);		//ＳＷ５６上の反転指定を行う(ﾊﾟﾀｰﾝﾃﾞｰﾀ,SW位置)
			SW16data_onset(0xff, 9);		//ＳＷ７の反転指定を行う(ﾊﾟﾀｰﾝﾃﾞｰﾀ,SW位置)
			break;
	}
...
#if (mot100_test == 1)
			mot_page_dat_chg();						//ﾓｰｼｮﾝ表示ﾍﾟｰｼﾞ(段数)にﾃﾞｰﾀを合わせる
#endif
...
//2017/08/10
////#if (mot100_test == 1)
////			LENG_svpv(143, SYO_LNG_CNT);							//小数点の変更(設定値&実測値)					位置決め完了範囲
////#endif
...
#if (mot100_test == 1)
					wk2b = 0;
					CSR_cls(143, wk2b, cBIT0);				//ｶｰｿﾙｸﾘｱ（位置決め完了範囲、反転ﾓｰﾄﾞP、反転ﾓｰﾄﾞS）
					SV_cls(143, wk2b, cBIT0);				//設定値ｸﾘｱ（位置決め完了範囲、反転ﾓｰﾄﾞP、反転ﾓｰﾄﾞS）
					PV_cls(143, wk2b, cBIT0);				//実測値ｸﾘｱ（位置決め完了範囲単位）
#else
...
#if (mot100_test == 1)
			mot_page_kaj_chg();								//各段の目標値が荷重の場合の単位切替（mm→kN）
#else
...
tanni_chg_skip:
...
#if (mot100_test == 1)
			MOT_NOSTEP_DISP_DI();					//不要行程の非表示処理
			Make_RepStep_Bit();						//リピート行程をBIT情報へ
			...
			mmon_page_char_clr();					//モーション画面  SW5：前頁、SW6：次頁　文字消去

			mmon_ud_char_clr();						//変速前／後消去		2016/04/19
...
//--------------------------------------------------------------------------------------------------------------
			...
			KAJ_svpv(128, SYO_KAJ_CNT);								//荷重計ｵｰﾊﾞﾛｰﾄﾞ最大値		2016/03/24
			KAJ_svpv(129, SYO_KAJ_CNT);								//荷重計ｵｰﾊﾞﾛｰﾄﾞ最小値		2016/03/24
			KAJ_svpv(130, SYO_KAJ_CNT);								//荷重計ｵｰﾊﾞﾛｰﾄﾞ振幅		2016/03/24
			KAJ_svpv(131, SYO_KAJ_CNT);								//荷重計ﾛｱｰﾘﾐｯﾄ最大値		2016/03/24
			KAJ_svpv(132, SYO_KAJ_CNT);								//荷重計ﾛｱｰﾘﾐｯﾄ最小値		2016/03/24
			KAJ_svpv(133, SYO_KAJ_CNT);								//荷重計ﾛｱｰﾘﾐｯﾄ振幅			2016/03/24
...
//			CSR_disp_cls(94);										//VS仕様のｶｰｿﾙ
		...
		case 24:		//設定画面　ﾛｰﾀﾘｶﾑ1～8ｶﾑ設定(進角有)					2014/09/01
			...
			else {												//タイマ対応			2016/11/15
				wk2b = cBIT0;
				for(cnt=0; cnt<8; cnt++, wk2b<<=1) {
					if (SEQ_039_SV & wk2b) {						//ﾀｲﾏ？
						CSR_buff[cnt].SV_MAX = 5;					//設定値を5個に減らす	OFF進角は設定不可
						SV_buff[cnt*8+6].B_COL1 = COL_WHITE;		//OFF進角背景色＝白
						SV_buff[cnt*8+7].B_COL1 = COL_WHITE;		//OFF進角(ﾀｲﾏ)背景色＝白
					}
				}
		...
		case 21:		//設定画面　ﾛｰﾀﾘｶﾑ1～8ｶﾑ設定
			...
			/*********	ロータリカムのタイマ選択処理	2016/11/15	*************/
			ROT_cam_tim(3, BIT_0);				//ﾛｰﾀﾘｶﾑ1
			ROT_cam_tim(4, BIT_1);				//ﾛｰﾀﾘｶﾑ2
			ROT_cam_tim(5, BIT_2);				//ﾛｰﾀﾘｶﾑ3
			ROT_cam_tim(6, BIT_3);				//ﾛｰﾀﾘｶﾑ4
			ROT_cam_tim(7, BIT_4);				//ﾛｰﾀﾘｶﾑ5
			ROT_cam_tim(8, BIT_5);				//ﾛｰﾀﾘｶﾑ6
			ROT_cam_tim(9, BIT_6);				//ﾛｰﾀﾘｶﾑ7
			ROT_cam_tim(10, BIT_7);				//ﾛｰﾀﾘｶﾑ8
...
////						SW_buff[cnt*3+2].SUB_NO |= BIT_15;				//OFF進角の単位(°／ms)を非表示
						SW_buff[cnt*4+2].SUB_NO |= BIT_15;				//ON進角の単位(°／ms)を非表示
						SW_buff[cnt*4+3].SUB_NO |= BIT_15;				//OFF進角の単位(°／ms)を非表示
					}
				}
			}
		...
		case 22:		//設定画面　ﾛｰﾀﾘｶﾑ9～16ｶﾑ設定
			...
			/*********	ロータリカムのタイマ選択処理	2016/11/15	*************/
			ROT_cam_tim(11, BIT_8);				//ﾛｰﾀﾘｶﾑ9
			ROT_cam_tim(12, BIT_9);				//ﾛｰﾀﾘｶﾑ10
			ROT_cam_tim(13, BIT_A);				//ﾛｰﾀﾘｶﾑ11
			ROT_cam_tim(14, BIT_B);				//ﾛｰﾀﾘｶﾑ12
			ROT_cam_tim(15, BIT_C);				//ﾛｰﾀﾘｶﾑ13
			ROT_cam_tim(16, BIT_D);				//ﾛｰﾀﾘｶﾑ14
			ROT_cam_tim(17, BIT_E);				//ﾛｰﾀﾘｶﾑ15
			ROT_cam_tim(18, BIT_F);				//ﾛｰﾀﾘｶﾑ16
...
//2015/01/26			SV0_SEND_POS	= GMN_PAR057*10;		//送り高さ
			...
			//--- レベラフィーダ ---						2016/03/18
			SV_cls(147, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ全般
			SV_cls(148, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ送り角度
			SV_cls(149, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ送り位置
			SV_cls(150, SV0_LFEED_SEL, BIT_0);				//レベラフィーダリリース角度
			SV_cls(151, SV0_LFEED_SEL, BIT_0);				//レベラフィーダリリース位置
			SV_cls(152, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ送り長さ
			SV_cls(153, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ矯正量
			PV_cls(147, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ全般
			CSR_cls(147, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ全般
			SW_cls(147, SV0_LFEED_SEL, BIT_0);				//レベラフィーダ全般

			if (SV_LFEED_FSK_SEL == 1) {					//
				wk2b = BIT_3;
			}
			else if (SV_LFEED_FSK_SEL == 2) {				//
				wk2b = BIT_4;
			}
			else {											//
				wk2b = BIT_2;
			}
			if ( (SV0_LFEED_SEL & BIT_0) == 0 ) {			//レベラフィーダ有無
				wk2b = 0;
			}
			SW_cls(144, wk2b, BIT_2);						//送り時間不足時 停止
			SW_cls(145, wk2b, BIT_3);						//送り時間不足時 待機
			SW_cls(146, wk2b, BIT_4);						//送り時間不足時 減速

			LENG_svpv(149, SYO_LNG_CNT);					//小数点の変更(設定値&実測値)	送り位置
			LENG_svpv(151, SYO_LNG_CNT);					//小数点の変更(設定値&実測値)	リリース位置
			LENG_svpv(152, SYO_LNG_CNT);					//小数点の変更(設定値&実測値)	送り長さ
			LENG_svpv(153, SYO_LNG_CNT);					//小数点の変更(設定値&実測値)	矯正量
			break;
...
//2014/12/22
		...
		case 110:		//多段モーション設定画面			2017/08/21
			LENG_svpv(91, SYO_LNG_CNT);						//小数点の変更(設定値&実測値)	上限位置
			LENG_svpv(155, SYO_LNG_CNT);					//小数点の変更(設定値&実測値)	多段開始／終了位置
			break;
	}
}
...
//2010/09/23			PV_cls(122, wk2b, BIT_7);								//電力量実測の非表示	//V06
		...
		case 25:		//生産画面　フィーダ設定	2016/03/24
			LENG_svpv(152, SYO_LNG_CNT);					//小数点の変更(設定値&実測値)	送り長さ
			LENG_svpv(153, SYO_LNG_CNT);					//小数点の変更(設定値&実測値)	矯正量
			break;
...
U2b		MOT_INF_GET(void)
{
	...
	U2b	stopcnt=0;
	U2b	stopcntmax=0;//A01t  : 2020/02/24
	U2b	ans=0;
	int	i;
...
/*
********************************************************************************
*  Function Name:	不要行程の非表示処理
*  Input	:		なし
*  Return	: 		なし
*  Note		:		
*
*  履 歴	:A.Shimamura		ON:2015-08-24
********************************************************************************
*/
void	MOT_NOSTEP_DISP_DI(void)
{
	UNs	dan, d_no, cnt, t_no, end_page;

	dan = SV_DANSUU_SRV - (SV_MOT_PAGE * 5);			//表示段数

	if (dan < 5) {
		/* 不要段のカーソル非表示 */
		d_no = 97 + dan;
		for (cnt=CSR_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( (CSR_buff[t_no].DATA_NO >= d_no) && (CSR_buff[t_no].DATA_NO <= 101) ) {		//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				CSR_buff[t_no].TOK_NO |= BIT_0;												//ｶｰｿﾙ非表示の設定
			}
		}
		...
		d_no = 86 + dan;
		for (cnt=SV_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( (SV_buff[t_no].GRP_NO >= d_no) && (SV_buff[t_no].GRP_NO <= 90) ) {		//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				SV_buff[t_no].SUB_NO |= BIT_F;											//設定値非表示の設定
			}
		}
		...
		d_no = 92 + dan;
		for (cnt=SV_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( (SV_buff[t_no].GRP_NO >= d_no) && (SV_buff[t_no].GRP_NO <= 96) ) {		//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				SV_buff[t_no].SUB_NO |= BIT_F;											//設定値非表示の設定
			}
		}
		...
		d_no = 97 + dan;
		for (cnt=SV_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( (SV_buff[t_no].GRP_NO >= d_no) && (SV_buff[t_no].GRP_NO <= 101) ) {	//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				SV_buff[t_no].SUB_NO |= BIT_F;											//設定値非表示の設定
			}
		}
		...
		d_no = 97 + dan;
		for (cnt=PV_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( (PV_buff[t_no].GRP_NO >= d_no) && (PV_buff[t_no].GRP_NO <= 101) ) {	//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				PV_buff[t_no].SUB_NO |= BIT_F;											//実測値非表示の設定
			}
		}
		...
		for (cnt=SW_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( (SW_buff[t_no].GRP_NO >= d_no) && (SW_buff[t_no].GRP_NO <= 101) ) {	//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				SW_buff[t_no].SUB_NO |= BIT_F;											//SW非表示の設定
			}
		}
	}
	...
	if (SV_MOT_PAGE < end_page) {
		/* 不要段のカーソル非表示 */
		for (cnt=CSR_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( CSR_buff[t_no].DATA_NO == 102 ) {					//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				CSR_buff[t_no].TOK_NO |= BIT_0;						//ｶｰｿﾙ非表示の設定
			}
		}
		...
		/* 不要段の設定値（速度、停止時間）非表示 */
		for (cnt=SV_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( SV_buff[t_no].GRP_NO == 102 ) {					//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				SV_buff[t_no].SUB_NO |= BIT_F;						//設定値非表示の設定
			}
		}
		...
		/* 不要段の実測値（単位）非表示 */
		for (cnt=PV_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( PV_buff[t_no].GRP_NO == 102 ) {					//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				PV_buff[t_no].SUB_NO |= BIT_F;						//設定値非表示の設定
			}
		}

		/* 不要段のスイッチ（停止Ｖ）非表示 */
		for (cnt=SW_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( SW_buff[t_no].GRP_NO == 102 ) {					//ｸﾞﾙｰﾌﾟ番号 ｻｰﾁ
				SW_buff[t_no].SUB_NO |= BIT_15;						//SW非表示の設定
			}
		}
	}
...
/*
********************************************************************************
*  Function Name:	モーション画面の表示行程No.イニシャル
*  Input	:		なし
*  Return	: 		なし
*  Note		:		
*
...
********************************************************************************
*/
void	SV_MOT_PAGE_INIT(void)
{
	SV_MOT_PAGE = 0;
	SV0_DISP_STEP_NO[0] = 1;
	SV0_DISP_STEP_NO[1] = 2;
	SV0_DISP_STEP_NO[2] = 3;
	SV0_DISP_STEP_NO[3] = 4;
	SV0_DISP_STEP_NO[4] = 5;
}
#endif
...
/*
********************************************************************************
*  Function Name:	モーション画面の表示行程No.+1
*  Input	:		なし
*  Return	: 		なし
*  Note		:		
*
...
********************************************************************************
*/
void	SV_MOT_PAGE_INC(void)
{
	SV_MOT_PAGE++;
	SV0_DISP_STEP_NO[0] += 5;
	SV0_DISP_STEP_NO[1] += 5;
	SV0_DISP_STEP_NO[2] += 5;
	SV0_DISP_STEP_NO[3] += 5;
	SV0_DISP_STEP_NO[4] += 5;
}
#endif
...
********************************************************************************
*/
void		Make_RepStep_Bit(void)
{
	UNs		rep_bit, cnt, wbit;
	UNs		step;

	if ( (SV_MOTREP_STGS == 0) ||						//開始行程=0
		 (SV_MOTREP_STGS > SV_DANSUU_SRV) ||			//開始行程>行程数
		 (SV_MOTREP_STGE > SV_DANSUU_SRV) ||			//終了行程>行程数
		...
		step = SV_MOT_PAGE * 5 + 1;					//ﾓｰｼｮﾝ表示ﾍﾟｰｼﾞ1行目の行程No.
		rep_bit = 0;
		wbit = cBIT0;

		for (cnt=0; cnt<5; cnt++, step++, wbit<<=1) {
			if ( (SV_MOTREP_STGS <= step) && (step <= SV_MOTREP_STGE) ) {
				rep_bit |= wbit;
			}
		}
...
/*
********************************************************************************
*  Function Name:	モーション画面  SW5：前頁、SW6：次頁　文字消去
*  Input	:		なし
*  Return	: 		なし
*  Note		:
*
...
********************************************************************************
*/
void	mmon_page_char_clr(void)
{
	UNs		cnt, t_no, end_page;

	if (SV_MOT_PAGE == 0) {
		for (cnt=CSR_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( CSR_buff[t_no].SW5_DN == 17 ) {					//前頁 文字ｻｰﾁ
				CSR_buff[t_no].SW5_DN = 0x0ff;
			}
		}
	}
	...
	if (SV_MOT_PAGE >= end_page) {
		for (cnt=CSR_CNT, t_no=0 ; cnt>0 ; cnt--, t_no++) {
			if ( CSR_buff[t_no].SW6_DN == 18 ) {					//次頁 文字ｻｰﾁ
				CSR_buff[t_no].SW6_DN = 0x0ff;
			}
		}
	}
}
...
/*
********************************************************************************
*  Function Name:	モーション画面  変速前／後 スイッチ消去
*  Input	:		なし
*  Return	: 		なし
*  Note		:
*
...
********************************************************************************
*/
void	mmon_ud_char_clr(void)
{
	UNs	cnt, t_no;

	if ((SV0_MOT_UDSEL & BIT_0) == 0) {						//ﾓｰｼｮﾝ変速選択機能無効？
		for (t_no=0; t_no<CSR_CNT; t_no++) {
			if (CSR_buff[t_no].SW56_UP == 97) {				//カーソル位置＝角度
				CSR_buff[t_no].SW5_DN = 0x0ff;
				CSR_buff[t_no].SW5_SUB = 0;
			}
		}

		for (t_no=0; t_no<SW_CNT; t_no++) {
			if (SW_buff[t_no].CARA_NO == 269) {				//SWｷｬﾗNo.＝269（前／後）
				SW_buff[t_no].SUB_NO |= BIT_15;				//SW非表示の設定
			}
		}
	}
