...
extern	UNs		GMN_PAR085;	//ﾓｰｼｮﾝ最大段数	2015-11-20 koma
...
#if (mot100_test == 1)
				if ( (PV0_CKAGL_OVBIT & bit) == 0 ) {			//角度リミット？
#else
...
//			SVSW_tbl_enzan();					//ＳＶＳＷテーブルアドレス演算
		...
		case 48:			//ｵﾌｾｯﾄ調整ｽﾀｰﾄ
			if ( (GAMEN_NO != 108) || ((GAMEN_NO == 108) && (SV0_OFSET_ENB == 1)) ) {	//機器交換画面では制限をかける		2016/02/24 
				OFF_set_start();					//ｵﾌｾｯﾄ調整ｽﾀｰﾄ
			}
//V05j			KJ_CHK_CNT1++;						//V05i
			break;
		case 49:			//ｹﾞｲﾝ調整ｽﾀｰﾄ
			if ( (GAMEN_NO != 108) || ((GAMEN_NO == 108) && (SV0_KOUSEI_ENB == 1)) ) {	//機器交換画面では制限をかける		2016/02/24 
				KOUSEI_start();						//校正ｽﾀｰﾄ
			}
...
//V02a				if( (SEQ_020_SV & BIT_8) && (SEQ_PAR_385 & BIT_7) )		//荷重計有＆荷重計表示指令有	/* V01m */
		...
		case 57:	/* V01d */		//モーション画面次項
			if (SV_DANSUU_SRV > 5) {
#if (mot100_test == 1)
				if (SV_MOT_PAGE < ((SV_DANSUU_SRV-1)/5)) {
					SV_MOT_PAGE_INC();
					GAMEN_disp();						//画面表示
					CSR_move(03,00);					//ｶｰｿﾙ下移動
...
//
		...
		case 108:	//レベラフィーダ  送り時間不足時	2015/03/21
			SV_LFEED_FSK_SEL++;
			if (SV_LFEED_FSK_SEL > 2) {
				SV_LFEED_FSK_SEL = 0;
			}
			...
			SW_cls_disp(144, wk_w, BIT_2);								//停止
			SW_cls_disp(145, wk_w, BIT_3);								//待機
			SW_cls_disp(146, wk_w, BIT_4);								//減速
		...
		case 109:					//モーション変速		2016/04/15
			if (SV0_MOT_UDSEL & BIT_0) {			//ﾓｰｼｮﾝ変速選択機能有効？
				wk_w = CSR_buff[CSR_TBL_POS_NOW].DATA_NO - 97;
				t_no = SV_MOT_PAGE * 5 + wk_w;		//変更があった行程No.0～99（=1～100）
				i = BIT_0;
				i <<= wk_w;
				SV0_MOT_BFAF ^= i;
				...
				wk_w = SV0_MOT_BFAF & i;
				step_chg_spd_set(t_no, wk_w);		//指定行程の変速設定（前or後）
		...
		case 110:			//ロータリカム  カム／タイマ切替				2016/11/14
			if(SEQ_022_SV & BIT_D){					//ﾛｰﾀﾘｶﾑ ﾀｲﾏ有無
				wk_w = CSR_buff[CSR_TBL_POS_NOW].SW56_UP & 0x000f;
				i = BIT_0;
				i <<= wk_w;
				SEQ_039_SV ^= i;					//変更ﾋﾞｯﾄﾃﾞｰﾀを今のﾃﾞｰﾀより反転する
...
//	2015-11-20 ｽﾀｯｸに多分入らない　ｸﾞﾛｰﾊﾞﾙに頭にwork_を追加
...
//2015-11-20	UNs	AGL[10];
//2015-11-20	UNl	POS[10];
//2015-11-20	UNs	SPD[10];
//2015-11-20	UNs	TIM[10];

//2015-11-20	UNs	AGL2[10];
//2015-11-20	UNl	POS2[10];
//2015-11-20	UNs	SPD2[10];
//2015-11-20	UNs	TIM2[10];
	...
	for (i=0; i<GMN_PAR085; i++) {	//2015-11-20koma 10->GMN_PAR085
		WORK_AGL[i]	=	Dst_AGL[i];
		WORK_POS[i]	=	Dst_POS[i];
		WORK_SPD[i]	=	Dst_SPD[i];
		WORK_TIM[i]	=	Dst_TIM[i];

		WORK_AGL2[i]	=	Dst_AGL[i];
		WORK_POS2[i]	=	Dst_POS[i];
		WORK_SPD2[i]	=	Dst_SPD[i];
		WORK_TIM2[i]	=	Dst_TIM[i];
	}

// koma 2012-10-19 5段仕様/10段仕様による段数
//2015-11-20	if ( SEQ_024_SV & BIT_3) {danmax=10;}else {danmax=5;}
...
//		=== タッチ挿入に同じデータが存在するかチェック =========
			...
			for(i=0;i<j;i++){
				aglwk1=WORK_AGL[i]/10;
				if((aglwk1==aglwk2) && (WORK_SPD[i]==kattch_spd)){
					pvdan=0;
				}
			}
			if(pvdan){//1
...
//		=== 減速に同じデータが存在するかチェック =========
		if(pvdanbef){
			...
			for(i=0;i<j;i++){
				aglwk1=WORK_AGL[i]/10;
				if((aglwk1==aglwk2) && (WORK_SPD[i]==spdwk)){
					pvdanbef=0;
				}
			}
...
//	==== 180度挿入 ====
		if(pvdanaft != 0){
			...
			for (i=0; i<GMN_PAR085; i++) {//2015-11-20koma 10->GMN_PAR085
				if(i==pvdanaft){
					WORK_AGL2[i]	=PVP_MOTINS_AG3;//金型ﾀｯﾁ後角度
					WORK_POS2[i]	=PVP_MOTINS_PS3;
					WORK_SPD2[i]	=kattch_spd;//100.00%=> 100.0% DATA
					WORK_TIM2[i]	=0;
					j=1;//挿入
				}else{
					WORK_AGL2[i]	=WORK_AGL[i-j];
					WORK_POS2[i]	=WORK_POS[i-j];
					WORK_SPD2[i]	=WORK_SPD[i-j];
					WORK_TIM2[i]	=WORK_TIM[i-j];
				}
			}
			for(i=0;i<GMN_PAR085;i++){//2015-11-20koma 10->GMN_PAR085
				WORK_AGL[i]	=WORK_AGL2[i];
				WORK_POS[i]	=WORK_POS2[i];
				WORK_SPD[i]	=WORK_SPD2[i];
				WORK_TIM[i]	=WORK_TIM2[i];
			}
		}
...
//	==== ﾀｯﾁ位置挿入 ====
		if(pvdan != 0){
			pvdan--;
			j=0;
			for (i=0; i<GMN_PAR085; i++) {//2015-11-20 koma 10->98
				if(i==pvdan){
					WORK_AGL2[i]	=PVP_MOTINS_AGL;//金型ﾀｯﾁ角度
					WORK_POS2[i]	=PVP_MOTINS_POS;
					WORK_SPD2[i]	=kattch_spd;//100.00%=> 100.0% DATA
					WORK_TIM2[i]	=0;
					j=1;
				}else{
...
//	==== 減速位置挿入 ====
		if(pvdanbef != 0){
			...
			for (i=0; i<GMN_PAR085; i++) {//2015-11-20 koma 10->98
				if(i==pvdanbef){
					...
					WORK_AGL2[i]	=PVP_MOTINS_AG2;//減速角度
					WORK_POS2[i]	=PVP_MOTINS_PS2;
					if(pvdanbef==0){
						WORK_SPD2[i]	=Dst_SPD[pvdanbef];//変更前の速度
					}else{
						WORK_SPD2[i]	=Dst_SPD[pvdanbef-1];
					}
					WORK_TIM2[i]	=0;
					j=1;
				}else{
		...
		//設定ﾃﾞｰﾀへ演算結果を戻す[]
		if(inscnt != 0){
			for (i=0; i<GMN_PAR085; i++) {//戻す 2015-11-20 koma 10->98
				Dst_AGL[i]	=	WORK_AGL[i];
				Dst_POS[i]	=	WORK_POS[i];
				Dst_SPD[i]	=	WORK_SPD[i];
				Dst_TIM[i]	=	WORK_TIM[i];
			}
