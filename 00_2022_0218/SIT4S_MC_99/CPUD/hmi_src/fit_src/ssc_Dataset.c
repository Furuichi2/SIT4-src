...
extern	UNs		GMN_PAR085;						//行程数最大							2016/09/20
extern	UNs		GMN_PAR019;						//振子の上限角リミット					2016/09/20
...
void	lfeed_sv_chg(UNs t_no);					//レベラフィーダ角度&位置の設定値変更		2016/03/24
void	SET_MS_CAL(void);						//多段モーション設定						2016/09/20
void	ms_sv_chg(UNs t_no);					//角度&位置の設定値変更						2016/09/20
...
//2014/09/04
						case E_key_sw7:			//key code SW7
							...
							else if (GMN_PAR095 == 1) {				//多段モーション設定有効				2016/09/17
								disp_stg_sel = E_dstg_msset;		//データ設定[多段モーション設定]へ
								GAMEN_NO = 110;
								GAMEN_disp();						//画面表示
							}
							break;
					...
					if(KEY_DATA_CODE == E_key_sw5){			//key code SW5
						...
						if ( SV_buff[SV_TBL_POS_NOW].SUB_NO == 53 ) {		//行程設定箇所				2014/09/08
							...
							if (GMN_PAR077 != 0) {								//繰り返し機能有り
								...
								//2020-09-01 SEI_test_out_sw5,6 sum演算対象
...
//							SEI_test_out_sw6();						//生産設定ﾃｽﾄ出力処理(SW6)
							...
							if( i < 0x80 || i >= 0x90 ){				//カム名称画面移動処理？
								...
								else if ( SV_buff[SV_TBL_POS_NOW].DSP_NO == 4 ) {		//OFF角度設定箇所			2016/11/15
									if (SEQ_022_SV & BIT_D) {							//タイマカム有効？
										/* カム／タイマ切替 */
										csr_sw56_sub(110);
										SWchg_disp();
										SW16data_onset(0xff, 0x08);				//ＳＷ１～６の反転指定を行う(ﾊﾟﾀｰﾝﾃﾞｰﾀ,SW位置)
				...
				case E_key_sw56_mode:		//SW5/6 key mode
					switch(KEY_DATA_CODE) {
						...
						case E_key_sw6:			//key code SW6	//ｽｷｯﾌﾟ
							...
							disp_stg_sel = E_dstg_init;			//データ設定[基本]へ			2016/09/30
							GAMEN_NO = 17;
							GAMEN_disp();						//画面表示
...
//2014/12/23							SEND_POS_2_AGL_CAL(SV0_SEND_POS);			//小松さん演算			2014/09/01
					...
					lfeed_sv_chg(i);							//レベラフィーダの角度／位置設定の変更処理	2016/03/24
					break;
				...
				case E_key_sw56_mode:		//SW5/6 key mode
					switch(KEY_DATA_CODE) {
						...
						case E_key_sw5:			//key code SW5		//2016/03/18
							SWchg_ent();							//SW CHG データ ENT
							SEI_test_out_sw5();						//2016/03/21
						...
						case E_key_sw5_down:	//key code SW5(立下り)			//2016/03/18
						case E_key_sw6_down:	//key code SW6(立下り)
							DANseq_alloff();						//段取り用シーケンスＯＦＦ
							SW16data_onset(0x00, 7);				//ＳＷ１～６の反転指定を行う(ﾊﾟﾀｰﾝﾃﾞｰﾀ,SW位置)
							SW16data_onset(0x00, 8);				//ＳＷ１～６の反転指定を行う(ﾊﾟﾀｰﾝﾃﾞｰﾀ,SW位置)
				...
				case E_key_sw56_mode:		//SW5/6 key mode
					switch(KEY_DATA_CODE) {
						...
						case E_key_sw4:			//key code SW4
							...
							break;
						case E_key_sw7:			//key code SW7
							if (GMN_PAR095 == 1) {					//多段モーション設定有効				2016/09/17
								disp_stg_sel = E_dstg_msset;		//データ設定[多段モーション設定]へ
								GAMEN_NO = 110;
								GAMEN_disp();						//画面表示
							}
		...
		case E_dstg_msset:		//データ設定[多段モーション設定]				2016/09/17
			switch(KEY_DATA_MODE) {
				case E_key_ent_mode:		//ENT key mode
					i = SV_TBL_POS_NOW;
					Key_Ent_sv();								//設定値の確定
					ms_sv_chg(i);								//角度&位置の設定値変更

					PV0_MSSET_STG1 = 0;							//設定ステージ初期化
					PV0_MSSET_MES1 = 0;							//メッセージ初期化
					SW56_dncara_disp(0xff, 7);
					SW56_dncara_disp(0xff, 8);
				...
				case E_key_sw56_mode:		//SW5/6 key mode
					switch(KEY_DATA_CODE) {
						...
						case E_key_sw5:			//key code SW5
							if (PV0_MSSET_STG1 == 1) {			//YES/NOステージ
								//NO押された
								PV0_MSSET_STG1 = 0;				//設定ステージ初期化
								PV0_MSSET_MES1 = 0;				//メッセージ初期化
								SW56_dncara_disp(0xff, 7);
						...
						case E_key_sw6:			//key code SW6
							if (PV0_MSSET_STG1 == 0) {
								SW56_dncara_disp(0xff, 7);
								SW56_dncara_disp(0xff, 8);
								SW56_dncara_disp(37, 7);			//ＳＷ７「NO」
								SW56_dncara_disp(38, 8);			//ＳＷ６「YES」
								PV0_MSSET_STG1 = 1;
								PV0_MSSET_MES1 = 22;				//確認メッセージ
							}
							else if (PV0_MSSET_STG1 == 1) {			//YES/NOステージ
								//YES押された
								SET_MS_CAL();						//多段設定をモーションへ反映
								PV0_MSSET_STG1 = 2;
								SW56_dncara_disp(0xff, 7);			//2016/10/14
								SW56_dncara_disp(0xff, 8);			//2016/10/14
							}
...
////
////#if (mot100_test == 1)
////	kata_load2(i, &SV_MOTDEG0_001);				//型データ(100段モーション)をFROMから読込み
////
////	SV_MOT_PAGE_INIT();
////#endif
////
...
/*
********************************************************************************
*  Module Name:		lfeed_sv_chg
*  Function Name:	レベラフィーダ角度&位置の設定値変更
*  Input	:		なし
*  Return	: 		なし
*  Note		:		角度→位置の設定値変更、或は 位置→角度の設定値変更を行う。
*
*  履 歴	:A.Shimamura		ON:2016/03/24
********************************************************************************
*/
void	lfeed_sv_chg(UNs t_no)
{
	UNl		long_bf;

	switch(SV_buff[t_no].GRP_NO) {
		case 148:				//送り角度
			long_bf = DigRnaChange1(SV_LFEED_STRDEG);					//角度→mm変換

			if(SEQ_050_SV & BIT_1) {									//反転　？
				long_bf = long_bf - SV_NEG_OFS_LNG;
			}

			SV_LFEED_STRPOS = cnv_mm_inch(long_bf, 0);					//単位切替(mm→mm/INCH)

			SVPV_data_disp(SV_LFEED_STRPOS, &SV_buff[t_no+1], 0, 0x01);	//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			SV_csr_set();												//設定値ｶｰｿﾙ表示
			break;

		case 149:				//送り位置
			long_bf = cnv_inch_mm(SV_LFEED_STRPOS, 0);					//単位切替(mm/INCH→mm)

			if(SEQ_050_SV & BIT_1) {		//反転　？
				long_bf = long_bf + SV_NEG_OFS_LNG;
			}

			SV_LFEED_STRDEG = RnaDigChange1(SV_LFEED_STRDEG, long_bf);	//mm変換→角度

			SVPV_data_disp(SV_LFEED_STRDEG, &SV_buff[t_no-1], 0, 0x01);	//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			break;

		case 150:				//リリース角度
			long_bf = DigRnaChange1(SV_LFEED_ENDDEG);					//角度→mm変換

			if(SEQ_050_SV & BIT_1) {									//反転　？
				long_bf = long_bf - SV_NEG_OFS_LNG;
			}

			SV_LFEED_ENDPOS	= cnv_mm_inch(long_bf, 0);					//単位切替(mm→mm/INCH)

			SVPV_data_disp(SV_LFEED_ENDPOS, &SV_buff[t_no+1], 0, 0x01);	//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			SV_csr_set();												//設定値ｶｰｿﾙ表示
			break;

		case 151:				//リリース位置
			long_bf = cnv_inch_mm(SV_LFEED_ENDPOS, 0);					//単位切替(mm/INCH→mm)

			if(SEQ_050_SV & BIT_1) {		//反転　？
				long_bf = long_bf + SV_NEG_OFS_LNG;
			}

			SV_LFEED_ENDDEG = RnaDigChange1(SV_LFEED_ENDDEG, long_bf);	//mm変換→角度

			SVPV_data_disp(SV_LFEED_ENDDEG, &SV_buff[t_no-1], 0, 0x01);	//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			break;
	}
}
...
/*
********************************************************************************
*  Module Name:		SET_MS_CAL
*  Function Name:	多段モーション設定
*  Input	:		なし
*  Return	: 		なし
*  Note		:		
*
*  履 歴	:A.Shimamura		ON:2016/09/20
********************************************************************************
*/
void	SET_MS_CAL(void)
{
	UNs		i, mot_cnt, rep_cnt;
	short	max, min;
	UNl		*pos;
	UNs		*agl, *spd, *tim;

	/* --- 多段モーションチェック ---*/
	if ( SV_MSSTAT_AGL < 1800 ) {						//多段開始角度が180°より小さい
		if ( SV_MSSTAT_AGL < SV_TAIKIKAKU_SRV) {		//多段開始角度は上限角より小さい
			PV0_MSSET_MES1 = 15;						//メッセージ表示「上限角、上限位置が動作範囲外」
			return;
		}
	}
	else {												//多段開始角度が180°より大きい
		if ( SV_MSSTAT_AGL > SV_TAIKIKAKU_SRV) {		//多段開始角度は上限角より大きい
			PV0_MSSET_MES1 = 15;						//メッセージ表示「上限角、上限位置が動作範囲外」
			return;
		}
	}

	if ( (SV_MSSTAT_AGL < 1800) && (SV_MSEND_AGL > 1800) ) {		//多段開始角度が180°より小さく、多段終了角度が180°より大きい
		//180°跨ぐ
		PV0_MSSET_MES1 = 16;							//メッセージ表示「多段範囲が0°または180°を跨いでいる
		return;
	}

	if ( (SV_MSSTAT_AGL > 1800) && (SV_MSEND_AGL < 1800) ) {		//多段開始角度が180°より大きく、多段終了角度が180°より小さい
		//0°跨ぐ
		PV0_MSSET_MES1 = 16;							//メッセージ表示「多段範囲が0°または180°を跨いでいる
		return;
	}

	if ( SEQ_050_SV & BIT_1 ) {							//現在反転
		PV0_MSSET_MES1 = 17;							//メッセージ表示「反転モーション選択中のため設定不可」
		return;
	}

	max = SV_TAIKIKAKU_SRV + SV_UPAREA_JUDG;
	if (max >= 3600) {
		max -= 3600;
	}
	min = SV_TAIKIKAKU_SRV - SV_UPAREA_JUDG;
	if (min < 0) {
		min += 3600;
	}
	if (min <= max) {
//2016/10/04
////		if ( ( (SV_MSSTAT_AGL >= min) && (SV_MSSTAT_AGL <= min) ) ||
////			 ( (SV_MSEND_AGL  >= max) && (SV_MSEND_AGL  <= max) ) ) {
		if ( ( (SV_MSSTAT_AGL >= min) && (SV_MSSTAT_AGL <= max) ) ||
			 ( (SV_MSEND_AGL  >= min) && (SV_MSEND_AGL  <= max) ) ) {
			PV0_MSSET_MES1 = 18;							//メッセージ表示「待機点範囲内に行程設定されるため設定不可」
			return;
		}
	}
	else {
		if ( (SV_MSSTAT_AGL >= min) || (SV_MSSTAT_AGL <= max) || (SV_MSEND_AGL  >= min) || (SV_MSEND_AGL  <= max) ) {
			PV0_MSSET_MES1 = 18;							//メッセージ表示「待機点範囲内に行程設定されるため設定不可」
			return;
		}
	}

	rep_cnt = SV_MS_CNT;							//多段回数（＝繰返し回数）
	if (rep_cnt == 0) {
		rep_cnt = 1;
	}
	mot_cnt = 2 * rep_cnt + 1;						//行程数決定
	max = GMN_PAR085;								//最大行程数
	if (max == 0) {
		max = 10;
	}
	if (mot_cnt > max) {
		PV0_MSSET_MES1 = 19;						//メッセージ表示「行程数不足」
		return;
	}

	/* --- モーション選択 ---*/
	i = SEQ_050_SV & ~0x000f;						//方式
//2016/10/30	if ( (SEQ_PAR_151 & BIT_6) && (GMN_PAR019 >= SV_TAIKIKAKU_SRV) ) {		//振子有効で上限角がパラメータ以下
	if ( (SEQ_PAR_151 & BIT_6) && (GMN_PAR019 <= SV_TAIKIKAKU_SRV) ) {		//振子有効で上限角がパラメータ以上
		/* 振子方式 */
		i |= BIT_2;									//振子選択
	}
	else {
	...
	/* --- 行程数の決定 ---*/
	SV_DANSUU_SRV = mot_cnt;							//行程数決定

	/* --- 各行程の目標角度の決定 ---*/
	SV_1DANPOS_SRV = SV_MSSTAT_POS;						//第1行程の目標位置
	SV_1DANDEG_SRV = SV_MSSTAT_AGL;						//第1行程の目標角度
	SV_1DANSPD_SRV = 1000;								//第1行程の目標速度
	SV_1DANTIM_SRV = 0;									//第1行程の停止時間

	pos = &SV_2DANPOS_SRV;
	agl = &SV_2DANDEG_SRV;
	spd = &SV_2DANSPD_SRV;
	tim = &SV_2DANTIM_SRV;
	for (i=2 ; i<=mot_cnt; i++, pos++, agl++, spd++, tim++) {
		if( (i%2) != 0 ) {
			/* 奇数行程*/
			//位置 = （多段開始位置－多段終了位置）÷多段回数×（多段回数－算出行程No）＋多段終了位置＋（多段開始位置－多段終了位置）÷多段回数×戻し量比率
			*pos = (SV_MSSTAT_POS - SV_MSEND_POS) / rep_cnt * (rep_cnt - i/2) + SV_MSEND_POS + (SV_MSSTAT_POS - SV_MSEND_POS) / rep_cnt * SV_MSBAK_COR / 100;
		}
		else {
			/* 偶数行程*/
			//位置 = （多段開始位置－多段終了位置）÷多段回数×（多段回数－算出行程No）＋多段終了位置
			*pos = (SV_MSSTAT_POS - SV_MSEND_POS) / rep_cnt * (rep_cnt - i/2) + SV_MSEND_POS;
		}
		
//2017/08/21		*agl = RnaDigChange1(SV_MSSTAT_AGL, *pos);		//角度←mm変換
		*agl = RnaDigChange1(SV_MSSTAT_AGL, cnv_inch_mm(*pos, 0));	//角度←mm変換
		*spd = SV_MSSTD_SPD;										//速度＝多段速度
		*tim = 0;													//停止時間＝0
	}
	for (; i<=99; i++, pos++, agl++, spd++, tim++) {
		*pos = 0;
		*agl = 0;
		*spd = 0;
		*tim = 0;
	...
	SV_TAIKSPD_SRV = 1000;								//戻り速度
	SV_TAIKTIM_SRV = 0;									//戻り停止時間
	...
	SV_MOT_CSUD_INF0 = 0xffff;				//加減速情報（1～16行程）
	SV_MOT_CSUD_INF1 = 0xffff;				//加減速情報（17～32行程）
	SV_MOT_CSUD_INF2 = 0xffff;				//加減速情報（33～48行程）
	SV_MOT_CSUD_INF3 = 0xffff;				//加減速情報（49～64行程）
	SV_MOT_CSUD_INF4 = 0xffff;				//加減速情報（65～80行程）
	SV_MOT_CSUD_INF5 = 0xffff;				//加減速情報（81～96行程）
	SV_MOT_CSUD_INF6 = 0xffff;				//加減速情報（97～100行程）
	...
	if (SEQ_050_SV & BIT_2) {
		PV0_MSSET_MES1 = 21;							//メッセージ表示「振子モーションで多段モーション設定しました。
	}
	else {
		PV0_MSSET_MES1 = 20;							//メッセージ表示「回転モーションで多段モーション設定しました。
	}
}
...
void	ms_sv_chg(UNs t_no)
{
	...
	switch(t_no) {
		...
		case 1:					//上限位置 -> 上限角算出
			...
			if(SEQ_050_SV & BIT_1) {									//反転？
				long_bf = long_bf + SV_NEG_OFS_LNG;
				SV_TAIKIKAKU_SRV = RnaDigChange1(SV_DNAREA_JUDG, long_bf);	//mm変換→角度
			}else{
				if(SEQ_050_SV & BIT_2) {								//振子？
		...
		case 2:					//多段開始角度 -> 開始位置算出
			SV_MSSTAT_POS = DigRnaChange1(SV_MSSTAT_AGL);				//角度→mm変換

			if(SEQ_050_SV & BIT_1) {									//反転？
				SV_MSSTAT_POS = SV_MSSTAT_POS - SV_NEG_OFS_LNG;
			}
			SV_MSSTAT_POS = cnv_mm_inch(SV_MSSTAT_POS, 0);				//単位切替(mm→mm/INCH)

			long_bf = SV_MSSTAT_POS;
			SVPV_data_disp(long_bf, &SV_buff[t_no+1], 0, 0x01);			//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			SV_csr_set();												//設定値ｶｰｿﾙ表示
			break;

		case 3:					//多段開始位置 -> 開始角度算出
			long_bf = cnv_inch_mm(SV_MSSTAT_POS, 0);					//単位切替(mm/INCH→mm)

			if(SEQ_050_SV & BIT_1) {									//反転？
				long_bf = long_bf + SV_NEG_OFS_LNG;
			}

			SV_MSSTAT_AGL = RnaDigChange1(SV_MSSTAT_AGL, long_bf);		//mm変換→角度

			long_bf = SV_MSSTAT_AGL;
			SVPV_data_disp(long_bf, &SV_buff[t_no-1], 0, 0x01);			//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			SV_csr_set();												//設定値ｶｰｿﾙ表示
			break;

		case 4:					//多段終了角度 -> 終了位置算出
			SV_MSEND_POS = DigRnaChange1(SV_MSEND_AGL);					//角度→mm変換

			if(SEQ_050_SV & BIT_1) {									//反転？
				SV_MSEND_POS = SV_MSEND_POS - SV_NEG_OFS_LNG;
			}
			SV_MSEND_POS = cnv_mm_inch(SV_MSEND_POS, 0);				//単位切替(mm→mm/INCH)

			long_bf = SV_MSEND_POS;
			SVPV_data_disp(long_bf, &SV_buff[t_no+1], 0, 0x01);			//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			SV_csr_set();												//設定値ｶｰｿﾙ表示
			break;

		case 5:					//多段開始位置 -> 開始角度算出
			long_bf = cnv_inch_mm(SV_MSEND_POS, 0);					//単位切替(mm/INCH→mm)

			if(SEQ_050_SV & BIT_1) {									//反転？
				long_bf = long_bf + SV_NEG_OFS_LNG;
			}

			SV_MSEND_AGL = RnaDigChange1(SV_MSEND_AGL, long_bf);		//mm変換→角度

			long_bf = SV_MSEND_AGL;
			SVPV_data_disp(long_bf, &SV_buff[t_no-1], 0, 0x01);			//設定値＆実測値 表示(ｾﾞﾛｻﾌﾟﾚｽあり/正常表示)
			SV_csr_set();												//設定値ｶｰｿﾙ表示
			break;
