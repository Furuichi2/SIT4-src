...
extern	void	COP1_INT_TIM_REF();//2013-12-25 MC
...
extern	UNs		GMN_PAR045;						//ｲﾝﾀｰﾛｯｸ表示ｷｬﾗｸﾀ色変更No.		SIT4-MC		2014/02/06
extern	S4b		RNA_STLORK;
extern	UNs		GMN_PAR077;						//繰り返し回数最大				2014/09/11
extern	UNs		GMN_PAR079;						//一定領域設定 有効／無効		2014/09/11
extern	UNs		GMN_PAR048;						//荷重制御有効／無効			2015/01/15
extern	UNs		GSHT2_RCV_CNT;					//2015/05/19
extern	UNs		GMN_PAR088;						//連続タイマ		2016/02/29
extern	UNs		GMN_PAR089;						//連続回数			2016/02/29
extern	UNs		GMN_PAR090;						//起動補償時間		2016/02/29
extern	UNs		GMN_PAR091;						//待機点１データ	2016/02/29
extern	UNs		GMN_PAR092;						//待機点２データ	2016/02/29
extern	UNs		GMN_PAR093;						//待機点３データ	2016/02/29
extern	UNs		GMN_PAR086;						//シーケンス２８	2016/02/29
extern	UNs		GMN_PAR087;						//シーケンス２９	2016/02/29
extern	UNs		GMN_PAR094;						//モード			2016/02/29
extern	UNs		SYS_FEED_PAR_BLK01;				//フィーダROMパラ	2016/03/21
extern	UNs		SYS_FEED_PAR0005;				//ﾌｨｰﾀﾞ機能有(1)／無(0)	2016/03/21
extern	UNs		SYS_FEED_PAR0010;				//ﾌｨｰﾀﾞ型式番号上限		2016/03/21

extern	UNs		GMN_PAR103;				//[1/2]高速ルーレット切換ＳＰＭ	2018-10-29(H2W 2017/06/20)
extern	UNs		GMN_PAR104;				//[1/2]低速ルーレット切換ＳＰＭ	2018-10-29(H2W 2017/06/20)
...
void	GAMEN_COPY(void);						//SIT4-MC
void	sinkaku_updn_chk(void);					//2014/09/02
void	GET_KAT_NAME_MEMO(void);				//2014/09/03
void	WAVE_TRND_DTP_CLR(void);				//2015/05/19
void	SYS_CBdata_read(void);					//シスパラよりクラッチ設定データ読み込み	2016/02/29
void	LFEED_main(void);						//レベラフィーダのメイン					2016/03/21
void	LFEED_init(void);						//レベラフィーダのイニシャル				2016/03/21
void	Get_LFEED_SYS_PARA(UNs no);				//レベラフィーダのROMパラ取得				2016/03/23
void	SEQ_ORIDE_CHG(void);					//ｼｰｹﾝｽからのｵｰﾊﾞﾗｲﾄﾞ値書換え				2016/04/22
UNs		ROT_cam_tim_init(void);					//ロータリカムタイマ仕様初期化				2016/11/15
...
extern	const	U4b		TFT_COL_PLDATA[64];			//SIT4-MC
extern	const	UNs *	KAT_sv_name_tbl[221];		//2014/09/03
extern	const	UNs *	KAT_sv_memo_tbl[221];		//2014/09/03
...
********************************************************************************
*/
extern	U2b FRAM_CBPV1_DAT01;//2020-09-17 debug
...
void	dsp_init1(void)
{
	...
	FRAM_csum_chk();				//ﾌﾗｼｭRAM領域の設定値のﾁｪｯｸｻﾑ･ｺｰﾄﾞのﾁｪｯｸ
	FRAM_KAT_csum_chk();			//ﾌﾗｼｭRAM領域の型ﾃﾞｰﾀ 名称＆メモのﾁｪｯｸｻﾑ･ｺｰﾄﾞのﾁｪｯｸ


	// V06  ========================================================
	INTR_DAT_OFS = 0x140000;
	poi = (UNs *)E_INTROCK_PAT;
	poi += INTR_DAT_OFS/2;
	chk = *poi;
	if (chk == 0xffff) {
		INTR_DAT_OFS = 0;
	...
	INIT_DAT_OFS = 0x140000;
	poi = (UNs *)E_FROM_INI_TOP;
	poi += INIT_DAT_OFS/2;
	chk = *poi;
	if (chk == 0xffff) {
		INIT_DAT_OFS = 0;
	...
	ALL_CHR_TBL_OFS = 0x140000;
	poi = &ALL_CHR_TBL;
	poi += ALL_CHR_TBL_OFS/2;
	chk = *poi;
	if (chk == 0xffff) {
		ALL_CHR_TBL_OFS = 0;
	}

//V06m	GAM_INF_TBL_OFS = 0x140000;
	GAM_INF_TBL_OFS = 0x140000 + 0x018000;
	poi = &GAM_INF_TBL;
	poi += GAM_INF_TBL_OFS/2;
	poi += 512/2;
	chk = *poi;
	chk &= 0xff00;
	if (chk != 0x5a00) {
	...
	//-------- KOMA ADD 2013-12-18 ----------------------------
	OP_STATUS_REAL=0x55;//work-mem
	MEM_ARI_REAL=0xA5;//work-mem
	
	//-------- cop1z有りから始まる[途中で無しにする] -----------
	...
	//2020-09-17 debug
//	if(500<FRAM_CBPV1_DAT01){
//		FRAM_CBPV1_DAT01=1;
//	}
//	SEQ_PAR_099=FRAM_CBPV1_DAT01;
//
...
//2014/03/13
	if ((short)SV_WAVE_SCL2U >= 0) {			//ｻﾝﾌﾟﾘﾝｸﾞ波形 最大荷重
		SMP_KAJ_MAX_SIN = 0x2b20;				//[+]
	}
	else {
		SMP_KAJ_MAX_SIN = 0x2d20;				//[-]
	}

	if ((short)SV_WAVE_SCL2D >= 0) {			//ｻﾝﾌﾟﾘﾝｸﾞ波形 最小荷重
		SMP_KAJ_MIN_SIN = 0x2b20;				//[+]
	}
	else {
		SMP_KAJ_MIN_SIN = 0x2d20;				//[-]
	}

	if ((short)SV_WAVE_SCL5U >= 0) {			//ｻﾝﾌﾟﾘﾝｸﾞ波形 最大電力
		SMP_EP_MAX_SIN = 0x2b20;				//[+]
	}
	else {
		SMP_EP_MAX_SIN = 0x2d20;				//[-]
	}

	if ((short)SV_WAVE_SCL5D >= 0) {			//ｻﾝﾌﾟﾘﾝｸﾞ波形 最小電力
		SMP_EP_MIN_SIN = 0x2b20;				//[+]
	}
	else {
		SMP_EP_MIN_SIN = 0x2d20;				//[-]
	}

	if ((short)SV_WAVE_SCL4U >= 0) {			//ﾄﾚﾝﾄﾞ波形 最大荷重
		TRD_KAJ_MAX_SIN = 0x2b20;				//[+]
	}
	else {
		TRD_KAJ_MAX_SIN = 0x2d20;				//[-]
	}

	if ((short)SV_WAVE_SCL4D >= 0) {			//ﾄﾚﾝﾄﾞ波形 最小荷重
		TRD_KAJ_MIN_SIN = 0x2b20;				//[+]
	}
	else {
		TRD_KAJ_MIN_SIN = 0x2d20;				//[-]
	}

	if ((short)SV_WAVE_SCL6U >= 0) {			//ﾄﾚﾝﾄﾞ波形 最大電力
		TRD_EP_MAX_SIN = 0x2b20;				//[+]
	}
	else {
		TRD_EP_MAX_SIN = 0x2d20;				//[-]
	}

	if ((short)SV_WAVE_SCL6D >= 0) {			//ﾄﾚﾝﾄﾞ波形 最小電力
		TRD_EP_MIN_SIN = 0x2b20;				//[+]
	}
	else {
		TRD_EP_MIN_SIN = 0x2d20;				//[-]
	}

	LFEED_init();								//レベラフィーダのイニシャル			2016/03/21
...
//V06n
	...
	SYS_CBdata_read();								//シスパラよりクラッチ設定データ読み込み	2016/03/02
	...
	RNA_STLORK = SV_RNA_STLORK;						//4;ストローク
	...
	if ( 2 < SV_KAJAREA_SEL ) {								//2014/03/17
		SV_KAJAREA_SEL = 0;
		sum_rem = 0xff;
	}
	...
	if ( GMN_PAR079 != 1 ) {								//一定領域設定 有効／無効		2014/09/10
		SV_MOT_FIX = 0;
		sum_rem = 0xff;
	}

	if (GMN_PAR048 != 1) {					//荷重制御有効？			2015/01/14
		/* 荷重制御無効 */
		SV_MOTOBJ_INF1		= 0;
	...
	if (SV_KAT_MM_INCH > 2) {				//型データ単位情報  0=mm 1=inch 2=inch高精度		2015/01/20
		SV_KAT_MM_INCH = 0;
		sum_rem = 0xff;
	}

	if ( ROT_cam_tim_init() ) {				//ロータリカムタイマ仕様初期化
		sum_rem = 0xff;
	}
...
#if (mot100_test == 1)
#else
	Make_Rep_Bit_Inf();								//リピート行程をBIT情報へ				2014/09/04
#endif
...
/* V01  */
	...
	IROCK_COL_CHG_NO = GMN_PAR045;					//SIT-4 MC		2014/02/06
	if (IROCK_COL_CHG_NO == 0) {
		IROCK_COL_CHG_NO = 88;
	}

	GET_KAT_NAME_MEMO();							//型ﾃﾞｰﾀ内の名称＆メモを読込む					2014/09/03

	if (GMN_PAR077 != 0) {							//繰り返し										2014/09/23
		SV0_REP_FIX_FLG |= cBIT0;
	}
	if (GMN_PAR079 == 1) {							//一定速度領域									2014/09/23
		SV0_REP_FIX_FLG |= cBIT1;
	}

	TANNI_SEQ_LST = SEQ_016_SV;						//単位情報格納									2015/01/20
}
...
void	dsp_time_5ms(void)
{
	...
	if(cop_enc_int_enb){//koma add
		COP1_MAIN_TLINK_DAT_REF();//2003-01-31// SAITO 2004-05-21

		//
		COP1_INT_TIM_REF();		//2013-12-25 MC
...
//V06b		if(EPM_SEC_MESMIN_CNT >= 60*5)					//5分積算更新
	...
	/*---	2014/09/25	---*/
	if(MOT_FERR_TIM) {								//F挿入ERR信号ON時間
		MOT_FERR_TIM--;
		SEQ_PAR_058 |= cBIT3;						//一定速度挿入異常 ON
	}
	else {
		SEQ_PAR_058 &= ~cBIT3;						//一定速度挿入異常 OFF
	}
...
void	dsp_main(void)
{
	...
	Any_mng();					/* 毎スキャンの固定処理 */
	Any_mng2();					/* 毎スキャンの固定処理 */		//2015/01/15

	GAMEN_COPY();				//SIT4-MC


	//2018-11-18 koma debug
//2020-09-17	SEQ_PAR_099=((SVSW_CHG_FLG<<8)&0xff00)|(CSTATUS_C&0x00ff);
	SEQ_PAR_085=SV_ATUOSU_D9;
...
#if (mot100_test == 1)
#else
//2015/07/10	モーションチェック機能
	PV0_CKSPD_LMBIT = PVP_CKSPD_LMBIT | ((PVP_CKSPD_LMBIT>>1) & BIT_14);
	if( ((GAMEN_NO == 3) && (SV_DANSUU_SRV > 5)) ||
		((GAMEN_NO == 28) && (SV_DANSUU_SRV > 5)) ) {
		PV0_CKSPD_LMBIT &= ~BIT_14;
	}
#endif
...
//2014/06/03
	SEQ_PAR_058 = (SEQ_PAR_058 & 0xf0ff) | ((SV_OVLD_SEL<<8) & 0x0300) | ((SV_LWLM_SEL<<10) & 0x0c00);

//2014/09/02
	sinkaku_updn_chk();								//進角が加減速範囲にいるかﾁｪｯｸ
...
void	Any_mng2(void)
{
	...
	if( SEQ_050_SV & BIT_1 ) {							//反転　？
		if (SEQ_PAR_151 & BIT_2) {						//目標荷重有効？
			SEQ_PAR_053 = SV_MOTOBJ_INF1;				/* 各段の目標 位置/荷重 */
		}
		else {
			SEQ_PAR_053 = 0;
		}
	...
	WAVE_TRND_DTP_CLR();								//2015/05/19

	if (SEQ_PAR_200 & cBIT15) {							//リセット？						2015/05/20
		SEQ_PAR_098 &= ~cBIT2;							//モーション挿入失敗フラグOFF
	}

	LFEED_main();										//レベラフィーダのメイン	2016/03/21

	SEQ_ORIDE_CHG();									//ｼｰｹﾝｽからのｵｰﾊﾞﾗｲﾄﾞ値書換え		2016/04/22
	TLINK_NEXT_READ();									//外部型番号読込処理				2016/04/22

	PV_NOW_ANGLE_RNA	= PVP_NOW_ANGLE_RNA;			//現在角度					2017/05/10
	PV_STOP_TIME		= PVP_STOP_TIM;					//ストッピングタイム		2017/05/10
	PV_BREAK_DIG		= PVP_BREAK_DIG;				//ブレーキ角度				2017/05/10
	PV_BREAK_TIM1		= PVP_BREAK_TIM1;				//ブレーキタイム			2017/05/10
}
...
/*
********************************************************************************
*  Function Name:	ﾛｰﾀﾘｶﾑ画面で進角が加減速範囲にいる情報に変化があれば設定値を再表示
*  Input	:		なし
*  Return	: 		なし
*  Note		:
********************************************************************************
*/
void	sinkaku_updn_chk(void)
{
	UNs		n1, n2, o1, o2;
	UNs		wk_w;

	n1 = PVP_SINKAKU_UPDN1;
	n2 = PVP_SINKAKU_UPDN2;
	o1 = PV0_SINKAKU_UPDN1;
	o2 = PV0_SINKAKU_UPDN2;
	PV0_SINKAKU_UPDN1 = n1;
	PV0_SINKAKU_UPDN2 = n2;

	if ( (GAMEN_NO == 24) || (GAMEN_NO == 25) ) {
		if (GAMEN_NO == 24) {
			n1 &= 0x00ff;
			o1 &= 0x00ff;
			n2 &= 0x00ff;
...
//2015/02/16	i = SEQ_020_SV & 0x4045;
	i = SEQ_020_SV & 0x40F5;
	j = ONOFF_LAST5;
	ONOFF_LAST5 = i;
	i ^= j;
...
////	}
	...
	if( (SEQ_020_SV & 0x00f0) == 0x0060 ) {		//2014/06/04
		/* 仕様設定69：ｽﾗｲﾄﾞ調節		OFF */
		/* 仕様設定70：ﾀﾞｲﾊｲﾄ表示		ON */
		/* 仕様設定71：ｽﾗｲﾄﾞ自動調節	ON */
		/* 仕様設定72：ｴﾝｺｰﾀﾞ16ﾋﾞｯﾄ 	OFF */
		SLDTUN_RNA_SEL = 0x5a;
	}
	else {
		SLDTUN_RNA_SEL = 0;
	...
	FRAM_csum_set_Spec();				//2020-09-01(_Spec) ﾌﾗｼｭRAM領域の設定値のﾁｪｯｸｻﾑ･ｺｰﾄﾞを作成
	if(ONOFF_FLG == 0xff){
...
//koma 2017-06-16	SEQ_PAR_099++;								//2015/09/20
//	2020-09-18
	if(SEQ_PAR_099<1000){
		SEQ_PAR_099++;
	}else{
...
//	***			2020-09-01	koma						***
//	***			ｻﾑ作成									***
//	***			ｻﾑﾁｪｯｸ異常が発生していた場合計算しない	***
//	***			ﾊﾟﾜ-ﾀﾞｳﾝ時								***
//	***													***
//	***													***
//	***													***
//	*******************************************************
void	FRAM_csum_set_Spec(void){

	if((SEQ_PAR_079&BIT_0)==0){
		FRAM_csum_set();
	}
...
void	FRAM_csum_chk(void)
{
	...
	if(SUM_CHK != scode ){						//設定値サムチェックコード チェック
		...
		SEQ_PAR_079 |= BIT_13;					//設定値ﾁｪｯｸｻﾑ異常
	}
}
...
void	FRAM_KAT_csum_set(void)
{
	...
	return;										//2015/11/26 型データ名称＆メモは、型データ内に入ったからサム不要とする！！！！！
...
//	*******************************************************
//	実際には領域をずらしたから不要
void	FRAM_KAT_csum_set_Spec(void){
...
//V06n
	...
	if(SUM_CHK_KAT != scode ){					//型ﾃﾞｰﾀ 名称＆メモ サムチェックコード
		SEQ_PAR_079 |= BIT_0;					//設定値ﾁｪｯｸｻﾑ異常
		SEQ_PAR_079 |= BIT_14;					//設定値ﾁｪｯｸｻﾑ異常
	}
...
//2020-09-18 電源offのｻﾑﾁｪｯｸ作成はやめる
//	FRAM_CBPV1_DAT01+=1;//2020-09-17 debug
//2020-08-15
//if(SUMCHK_PCAL_OFF!=DEBUG_YES)
//										//2020-09-01 _Specに変更
//	FRAM_csum_set_Spec();				//ﾌﾗｼｭRAM領域の設定値のﾁｪｯｸｻﾑ･ｺｰﾄﾞを作成
//	FRAM_KAT_csum_set_Spec();			//ﾌﾗｼｭRAM領域の型ﾃﾞｰﾀ 名称＆メモのﾁｪｯｸｻﾑ･ｺｰﾄﾞを作成
//
//endif
...
//2019-06-07 koma
	...
	if ( (CB_SYS_PARAM045 == 1) && (SEQ_050_SV & BIT_0) ) {
		w2b = 0;
...
//V01m_a				SV_TAIKIPOS_SRV	= cnv_mm_inch(SV_TAIKIPOS_SRV, 0);			//単位切替(mm→mm/INCH)
		...
		TAIKIKAKU_CHG_FLG =0;//2019-06-07 koma 
	}
}
...
void	DIE_HEIGHT_OFFSET(void)
{
	...
	if (seq_one & cBIT10) {				//＋オフセット指令
		ofs = (S4b)GMN_PAR020;
		ofs *= 1000;					//小数点3桁へ
//2015/02/16		ofs = cnv_mm_inch(ofs, 2);
...
/*
********************************************************************************
*  Module Name:		GAMEN_COPY
*  Function Name:	表示画面のコピー
*  Input	:		なし
*  Return	: 		なし
*  Note		:
*
...
void	GAMEN_COPY(void)
{
	...
	seq_chg = seq_bit = SEQ_PAR_151;				// new
	seq_chg ^= GAM_COPY_FLG;						// new ^ old = chg
	seq_one = seq_chg & seq_bit;					// chg & new = one
	seq_ofe = seq_chg & GAM_COPY_FLG;				// chg & old = ofe
	GAM_COPY_FLG = seq_bit;

	if (seq_one & cBIT1) {
//		VRAM_Dst = (UNs *)0xAC300000;
		VRAM_Dst8 = (UNb *)0xAC300000;
		VRAM_Src = &LCD_VRAM[0][0];
		VRAM_End = VRAM_Src + (640*480) -1;
		GAM_COPY_STS |= cBIT0;						//COPY中ON
	}

//ためしkoma	if (GAM_COPY_STS & cBIT0) {
//ためしkoma		for(cnt=0;cnt<(640*480);cnt++){
//ためしkoma			no = *VRAM_Src;
//ためしkoma			*VRAM_Dst8 = no;
//ためしkoma		}
//ためしkoma		GAM_COPY_STS |= cBIT1;				//完了ON
//ためしkoma	}

	if (GAM_COPY_STS & cBIT0) {
		for (cnt=0; cnt<0x2000; cnt++, VRAM_Dst8++, VRAM_Src++) {
			no = *VRAM_Src;
//			dat24 = TFT_COL_PLDATA[no];
//			dat16 = ((dat24>>3)&0x001f) | ((dat24>>5)&0x07e0) | ((dat24>>8)&0xf800);
//			*VRAM_Dst = dat16;
			*VRAM_Dst8 = no;
			if (VRAM_Src >= VRAM_End) {
				GAM_COPY_STS &= ~cBIT0;				//COPY中OFF
				GAM_COPY_STS |= cBIT1;				//完了ON
				break;
			}
		}
	}
...
/*
********************************************************************************
*  Function Name:	型ﾃﾞｰﾀ内の名称＆メモを読込み
*  Input	:		なし
*  Return	: 		なし
*  Note		:
*
*  履 歴	:Shima			ON:2014/09/03
********************************************************************************
*/
void	GET_KAT_NAME_MEMO(void)
{
	UNs		*kat_name, *kat_memo, *dst;
	UNs		cnt;

	kat_name = (UNs *)KATA_DATA_ADR_TOP;
	kat_name += 0x700/2;
	kat_memo = kat_name;
	kat_memo += 0x10/2;

	for (cnt=1; cnt<=100; cnt++, kat_name+=0x800/2, kat_memo+=0x800/2) {
		dst = KAT_sv_name_tbl[cnt];				//型ﾃﾞｰﾀ 名称領域のｱﾄﾞﾚｽ･ﾃｰﾌﾞﾙ
		memcpy_word(dst, kat_name, 8);			//型ﾃﾞｰﾀ 内の型名称
		dst = KAT_sv_memo_tbl[cnt];				//型ﾃﾞｰﾀ メモ領域のｱﾄﾞﾚｽ･ﾃｰﾌﾞﾙ
		memcpy_word(dst, kat_memo, 8);			//型ﾃﾞｰﾀ 内の型メモ
	}
	...
	for (; cnt<=220; cnt++, kat_name+=0x800/2, kat_memo+=0x800/2) {
		dst = KAT_sv_name_tbl[cnt];				//型ﾃﾞｰﾀ 名称領域のｱﾄﾞﾚｽ･ﾃｰﾌﾞﾙ
		memcpy_word(dst, kat_name, 8);			//型ﾃﾞｰﾀ 内の型名称
		dst = KAT_sv_memo_tbl[cnt];				//型ﾃﾞｰﾀ メモ領域のｱﾄﾞﾚｽ･ﾃｰﾌﾞﾙ
		memcpy_word(dst, kat_memo, 8);			//型ﾃﾞｰﾀ 内の型メモ
...
********************************************************************************
*/
void	WAVE_TRND_DTP_CLR(void)
{
	UNs		seq_chg, seq_bit, seq_one, seq_ofe;

	seq_chg = seq_bit = SEQ_PAR_056;				// new
	seq_chg ^= TRND_DTP_FLG;						// new ^ old = chg
	seq_one = seq_chg & seq_bit;					// chg & new = one
	seq_ofe = seq_chg & TRND_DTP_FLG;				// chg & old = ofe
	TRND_DTP_FLG = seq_bit;

	if (seq_one & cBIT11) {							// 56.11 on
		WAVE_TRND_DTP[0] = 0;
		GSHT2_RCV_CNT = 0;
	}
}
...
/*
********************************************************************************
*  Module Name:		SYS_CBdata_read
*  Function Name:	シスパラよりクラッチ設定データ読み込み
*  Input	:		なし
*  Return	: 		なし
*  Note		:		シスパラよりクラッチ設定データ読み込み
*
*  履 歴	:Shima		ON:2016/03/02
********************************************************************************
*/
void SYS_CBdata_read(void)
{
	SV_RNZTIM_A12	= GMN_PAR088;			//連続タイマ
	SV_RNZRPM_A12	= GMN_PAR089;			//連続回数
	SV_HOSTIM_A12	= GMN_PAR090;			//起動補償時間
	SV_TAIKI1_A12	= GMN_PAR091*10;		//待機点１データ
	SV_TAIKI2_A12	= GMN_PAR092*10;		//待機点２データ
	SV_TAIKI3_A12	= GMN_PAR093*10;		//待機点３データ

	SEQ_028_SV		= GMN_PAR086;			//シーケンス２８
	SEQ_029_SV		= GMN_PAR087;			//シーケンス２９

	SV_PSDIMD_A1	= GMN_PAR094;			//モード

	/* 2018-10-29 [1/2] */
	SYS_HIGHSPM_B1	= GMN_PAR103;			//高速ルーレット切換ＳＰＭ			2017/06/20
	SYS_LOWSPM_B1	= GMN_PAR104;			//低速ルーレット切換ＳＰＭ			2017/06/20
}


/*
********************************************************************************
*  Function Name:	レベラフィーダのメイン
*  Input	:		なし
*  Return	: 		なし
*  Note		:
*
...
********************************************************************************
*/
void	LFEED_main(void)
{
	UNs	flg;
	UNl	dat;

	PV0_LFEED_POS = cnv_mm_inch(PVP_LFEED_POS*10, 1);		//レベラフィーダ現在位置

	flg = 0;
	if (SEQ_PAR_153 & BIT_0) {					//同期運転無効指令
		if (SV_LFEED_GMNSQ1 & BIT_0) {
			SV_LFEED_GMNSQ1 &= ~BIT_0;
			flg = 0xff;
		}
	}

	if (SEQ_PAR_153 & BIT_1) {					//リリース無効指令
		if (SV_LFEED_GMNSQ1 & BIT_1) {
			SV_LFEED_GMNSQ1 &= ~BIT_1;
			flg = 0xff;
		}
	}
...
********************************************************************************
*/
void	LFEED_init(void)
{
	if ((short)SV_LFEED_HOSIN >= 0) {			//ﾚﾍﾞﾗﾌｨｰﾀﾞ矯正量入口
		SV0_LFEED_HSIS = 0x2b20;				//[+]
	}
	else {
		SV0_LFEED_HSIS = 0x2d20;				//[-]
	}

	if ((short)SV_LFEED_HOSOT >= 0) {			//ﾚﾍﾞﾗﾌｨｰﾀﾞ矯正量出口
		SV0_LFEED_HSOS = 0x2b20;				//[+]
	}
	else {
		SV0_LFEED_HSOS = 0x2d20;				//[-]
	}

	if (SYS_FEED_PAR0005 == 1) {					//レベラフィーダ有り？
		SV0_LFEED_SEL = BIT_0;
		Get_LFEED_SYS_PARA(SV_LFEED_SELNO);			//レベラフィーダのROMパラ取得
	}
}
...
********************************************************************************
*/
void	Get_LFEED_SYS_PARA(UNs no)
{
	UNs		*LF_WK, i;

	if (no > SYS_FEED_PAR0010) {					//型式番号最大値超えてる？
		no = 1;										//デフォルト
	}
	if (no>0) {
		no--;
	...
	LF_WK = &SYS_FEED_PAR_BLK01;
	LF_WK += no * 32;
	for (i=0; i<32; i++) {							//ROMパラ32ワード取得
		SYS0_FEED_PAR_BLK[i] = LF_WK[i];
	}

	LF_WK = (UNs *)&PV0_LFEED_TYPE[0];
	for (i=0; i<8; i++) {							//フィーダ型式ASCII 16文字取得
		LF_WK[i] = SYS0_FEED_PAR_BLK[i];
	}
}
...
/****************************************************************/
/*																*/
/*			ｼｰｹﾝｽからのｵｰﾊﾞﾗｲﾄﾞ値書換え							*/
/*																*/
/*			A.Shima				2016/04/22						*/
/*																*/
/****************************************************************/
void	SEQ_ORIDE_CHG(void)
{
	UNs		bak, flg, chg, one, dat;

	bak = SQ_ORIDE_BAK;							// 指令前回値
	flg = SEQ_PAR_144;						// 指令今回値

	SQ_ORIDE_BAK = flg;							// 更新
	chg = bak^flg;								// 変化ﾋﾞｯﾄMAKE
	one = chg&flg;								// 立ち上がりﾋﾞｯﾄMAKE

	if( one & BIT_7)							// 変更指令143.0立ち上がり？
	{
		dat = SEQ_PAR_144 & 0x7f;					// 指令ﾃﾞｰﾀ
		if( (1<=dat) && (dat<=100) )
		{
			SV_OVERLIDE_COF = dat;					// 設定値書換え
			SVSW_CHG_FLG |= 0x0004;
			FRAM_csum_set();						//ﾌﾗｼｭRAM領域の設定値のﾁｪｯｸｻﾑ･ｺｰﾄﾞを作成
			if( (GAMEN_NO == 1) || (GAMEN_NO == 17) )	// 生産画面ｏｒデータ設定画面
			{
				SVdata_disp();							//設定値情報　表示
			}
		}
	...
	SEQ_PAR_096 = (SV_OVERLIDE_COF & 0x7F) | (flg & BIT_7);				// 応答
...
UNs		ROT_cam_tim_init(void)
{
	...
	if ( (SEQ_022_SV & BIT_D) == 0 ) {					//カムタイマ仕様無効
		SEQ_039_SV = 0;
		
		Dst_p = &SV_RCLSTA11_C17T;						//ロータリカム１　ＯＮ工程　タイマ仕様
		for (i=0; i<4*16; i++) {
			Dst_p[i] = 0;
		}
