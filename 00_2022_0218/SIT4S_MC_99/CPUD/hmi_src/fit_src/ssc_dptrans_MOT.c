...
//2016/03/21	レベラフィーダ設定関連
	SVP_LFEED_LNG1		= SV_LFEED_LNG1			;//送り長さ　0.01mm
	DPB_SEND_4B(&SVP_LFEED_LNG1);
	SVP_LFEED_STRPOS	= SV_LFEED_STRPOS		;//送り開始位置 0.001mm
	DPB_SEND_4B(&SVP_LFEED_STRPOS);
	SVP_LFEED_STRDEG	= SV_LFEED_STRDEG		;//送り開始角度 0.1度
	DPB_SEND_2B(&SVP_LFEED_STRDEG);
	SVP_LFEED_STRNO		= SV_LFEED_STRNO		;//送り開始行程		
	DPB_SEND_2B(&SVP_LFEED_STRNO);
	SVP_LFEED_ENDPOS	= SV_LFEED_ENDPOS		;//送り終了位置 0.001mm
	DPB_SEND_4B(&SVP_LFEED_ENDPOS);
	SVP_LFEED_ENDDEG	= SV_LFEED_ENDDEG		;//送り終了角度 0.1度
	DPB_SEND_2B(&SVP_LFEED_ENDDEG);
	SVP_LFEED_ENDNO		= SV_LFEED_ENDNO		;//送り終了行程
	DPB_SEND_2B(&SVP_LFEED_ENDNO);
	SVP_LFEED_SPD1		= SV_LFEED_SPD1			;//通常の送り速度1%
	DPB_SEND_2B(&SVP_LFEED_SPD1);
	SVP_LFEED_SPD2		= SV_LFEED_SPD2			;//低速の送り速度1%
	DPB_SEND_2B(&SVP_LFEED_SPD2);
	SVP_LFEED_SELNO		= SV_LFEED_SELNO		;//型式の選択番号
	DPB_SEND_2B(&SVP_LFEED_SELNO);
	SVP_LFEED_HOSIN		= SV_LFEED_HOSIN		;//矯正量(ｸﾘｱﾗﾝｽ) 入口
	DPB_SEND_2B((UNs *)&SVP_LFEED_HOSIN);
	SVP_LFEED_HOSOT		= SV_LFEED_HOSOT		;//矯正量(ｸﾘｱﾗﾝｽ) 出口
	DPB_SEND_2B((UNs *)&SVP_LFEED_HOSOT);
	...
	SVP_LFEED_GMNSQ1	= SV_LFEED_GMNSQ1 & ~(BIT_4+BIT_3+BIT_2) | wk_w;		;//ｼｰｹﾝｽ396
	DPB_SEND_2B(&SVP_LFEED_GMNSQ1);

	SVP_LFEED_GMNSQ2	= SV_LFEED_GMNSQ2		;//ｼｰｹﾝｽ397
	DPB_SEND_2B(&SVP_LFEED_GMNSQ2);
	SVP_LFEED_GMNSQ3	= SV_LFEED_GMNSQ3		;//ｼｰｹﾝｽ398
	DPB_SEND_2B(&SVP_LFEED_GMNSQ3);
	SVP_LFEED_GMNSQ4	= SV_LFEED_GMNSQ4		;//ｼｰｹﾝｽ399
	DPB_SEND_2B(&SVP_LFEED_GMNSQ4);
