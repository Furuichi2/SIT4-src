...
void	Dsp_PvSamp_Wave(void)
{
	...
	UNs		ydiff, ybak;
	UNs		i, j;
	int		s_dtl, s_ulml, s_dlml, s_lenl;
...
//////			ybak = WAVE_SAMP_DOT[poi_old].elepw;		//ydot前回位置
			...
			s_ulml = (int)SV_WAVE_SCL5U;						//ｸﾞﾗﾌの上限＝機械上限
			s_dlml = (int)SV_WAVE_SCL5D * -1;					//ｸﾞﾗﾌの下限＝機械下限
			s_dtl = (int)WAVE_SAMP_DATA[poi_old+1].elepw;		//実測
...
//	-----------------------------------------------------------------------------------
	...
	else
	{
		...
		/**************** 1ｻｲｸﾙ電力量の波形描画 ******************/
		WAVE_TRND_DATA[poi].elepw = WAVE_TRND_DATA[poi-1].elepw +100;
		if (WAVE_TRND_DATA[poi].elepw > 10000) {
			WAVE_TRND_DATA[poi].elepw = 0;
		}
	}
